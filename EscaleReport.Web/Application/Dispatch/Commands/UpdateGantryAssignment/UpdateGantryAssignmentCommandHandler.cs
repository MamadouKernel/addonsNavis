...
public class UpdateGantryAssignmentCommandHandler(
    IApplicationDbContext dbContext,
    ICurrentUserService currentUser) : IRequestHandler<UpdateGantryAssignmentCommand>
{
    public async Task Handle(UpdateGantryAssignmentCommand request, CancellationToken cancellationToken)
    {
        ...
        assignment.CorrigerHeureDebut(request.HeureDebut);
        if (request.HeureFin.HasValue)
        {
            assignment.Terminer(request.HeureFin);
            var gantry = await dbContext.Gantries
                .FirstOrDefaultAsync(g => g.Id == assignment.GantryId, cancellationToken);
