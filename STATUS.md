...
Tous les tableaux qui accumulent un historique non borné (croissent sans limite sur la durée de vie du terminal) sont désormais paginés, 20 lignes par page, via une infrastructure partagée (`PagedResult<T>`/`Paging.Resolve` dans `Application/Common/Models`, partial `_Pagination.cshtml`) : Escales, Cargo Control, Journal d'audit (pagination directement en base, remplace l'ancien plafond fixe de 500 lignes), Yard Planner (4 tableaux), Coordinateur (Synthèse navires/Cargo, Incidents), ITT Controller (Transferts/Incidents/Pannes), Dispatch STS/TT/RTG/Autres engins (14 tableaux au total), sous-onglets Vessel Planning de la fiche escale (Anomalies/Vides/Incidents/Additionnels/Dangereux). Chaque tableau d'une même page a son propre paramètre de page en query string (ex. `?assignmentsPage=2&incidentsPage=1`), pour que la pagination d'un tableau ne perturbe pas les autres. Un numéro de page invalide (0, négatif, au-delà du nombre réel de pages) est silencieusement ramené dans les bornes plutôt que de planter ou d'afficher une page vide.

Les compteurs et totaux affichés à côté de ces tableaux (pastilles d'onglet "en cours"/"non résolu", et pour les conteneurs vides les 6 totaux souhaité/ajouté/planifié/embarqué/coupé/restant) sont calculés côté serveur sur l'ensemble des enregistrements avant découpage en pages — ils restent exacts quelle que soit la page affichée, y compris pour le bandeau d'alerte "points d'attention non traités" affiché à la clôture d'une escale (CDC §4.3), qui reposait sur ces mêmes totaux.

Point d'attention traité : `GetEscaleDetailQuery` est aussi utilisé par la génération du rapport PDF (§14.3) et de l'export Excel (§14.4), qui ont besoin de la liste complète de chaque catégorie — un rapport imprimé ne peut pas se limiter à la page affichée à l'écran au moment du clic. Un indicateur `Unbounded` sur la requête (positionné par ces deux générateurs uniquement) fait remonter tous les enregistrements sans pagination, pour ces deux cas d'usage spécifiquement.

La page Statistiques (§17) ne contient aucun tableau brut (uniquement des indicateurs de synthèse) : aucune pagination n'y est nécessaire.

Sciemment **non paginés** : listes intrinsèquement bornées ou pensées pour être vues en une fois — Utilisateurs (borné par l'effectif), Alertes actives (auto-purgées à mesure que les situations se résolvent), Rapport de fin de shift (photo d'un instant, faite pour être lue/imprimée en entier), sous-onglets Vessel Planning d'une escale (Anomalies/Vides/Incidents/Additionnels/Dangereux — liste de travail propre à une seule escale, à garder visible en entier).
...
| Pas de HTTPS/HSTS ni d'en-têtes de sécurité | `UseHttpsRedirection`/`UseHsts` + CSP, X-Frame-Options, X-Content-Type-Options, Referrer-Policy, Permissions-Policy dans `Program.cs`. **Déploiement IIS local prévu en HTTP pur** (pas de certificat) : ces trois mécanismes sont désormais pilotés par `Security:RequireHttps` (config), positionné à `false` dans `appsettings.Production.json` — le même binaire publié s'adapte sans recompilation |
| Cookie de session sans `Secure` forcé | `CookieSecurePolicy.Always` quand `Security:RequireHttps` est vrai (Dev et le déploiement IIS-HTTP prévu utilisent `SameAsRequest`, faute d'endpoint https) |
| Compte désactivé par un Administrateur : une session déjà ouverte restait pleinement utilisable (IsActive n'est vérifié qu'à la connexion) | `UsersController.ToggleActive` change le security stamp de l'utilisateur désactivé (`UpdateSecurityStampAsync`) ; `SecurityStampValidatorOptions.ValidationInterval` réduit à 2 min (`Program.cs`, 30 min par défaut) — la session est donc coupée dans les 2 minutes suivant la désactivation, au lieu de rester valide jusqu'à expiration du cookie (8h) |
